 ...
 * distribution, MPI_THREAD_MULTIPLE mode using separate windows for the submatrices A, B,
 * and C stored in a process. All threads share the same window for each of the submatrices.
 *
...
int main(int argc, char **argv)
{
    ...
    int tot_non_zero_tiles, tiles_in_my_rank;
    int p_dim, node_dim;
    int ppn;
    ...
    double *win_a_mem, *win_b_mem, *win_c_mem;
    int *counter_win_mem;
    MPI_Win win_a, win_b, win_c, win_counter;
    ...
    tiles_in_my_rank = tot_non_zero_tiles / nprocs;
    if (rank < (tot_non_zero_tiles % nprocs))
        tiles_in_my_rank++;
#if DEBUG
    printf("Rank %d: Non-zero tiles with me %d\n", rank, tiles_in_my_rank);
#endif
...
#if DEBUG
    if (rank == 0) printf("Set info hints\n");
#endif
#endif

    /* Allocate and create RMA windows for the tiles in A, B, and C */
    sub_mat_elements = elements_in_tile * tiles_in_my_rank;
    MPI_Win_allocate(sub_mat_elements * sizeof(double), sizeof(double),
                    MPI_INFO_NULL, MPI_COMM_WORLD, &win_a_mem, &win_a);
    MPI_Win_allocate(sub_mat_elements * sizeof(double), sizeof(double),
                    MPI_INFO_NULL, MPI_COMM_WORLD, &win_b_mem, &win_b);
    MPI_Win_allocate(sub_mat_elements * sizeof(double), sizeof(double),
#if OFI_WINDOW_HINTS
                    win_info,
#else  
    ...
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, MPI_MODE_NOCHECK, win_a);
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, MPI_MODE_NOCHECK, win_b);
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, MPI_MODE_NOCHECK, win_c);
    init_sub_mats(sub_mat_a, sub_mat_b, sub_mat_c, sub_mat_elements);
    MPI_Win_unlock(rank, win_a);
    MPI_Win_unlock(rank, win_b);
    ...
    MPI_Win_lock_all(MPI_MODE_NOCHECK, win_a);
    MPI_Win_lock_all(MPI_MODE_NOCHECK, win_b);
    ...
    /* Warmup */
#if DEBUG
    if (rank == 0) printf("Starting warmup!\n");
#endif
#pragma omp parallel
    {
        int work_id;
...
#pragma omp barrier
        ...
        do {
            ...
            if (target_rank_a == rank) {
                ...
                /* get tile A */
                MPI_Get(local_ta, elements_in_tile, MPI_DOUBLE, target_rank_a, target_offset_a, elements_in_tile, MPI_DOUBLE, win_a);
                MPI_Win_flush(target_rank_a, win_a);
            }
            ...
            if (target_rank_b == rank) {
                ...
                /* get tile B */
                MPI_Get(local_tb, elements_in_tile, MPI_DOUBLE, target_rank_b, target_offset_b, elements_in_tile, MPI_DOUBLE, win_b);
                MPI_Win_flush(target_rank_b, win_b);
            }
...
#if DEBUG
    printf("Rank %d done with the warmup\n", rank);
#endif
    MPI_Barrier(MPI_COMM_WORLD);
 #if DEBUG
    printf("Rank %d finished barrier after warmup\n", rank);
#endif
    ...
    MPI_Barrier(MPI_COMM_WORLD);
#if DEBUG
    printf("Rank %d finished barrier just before benchmark\n", rank);
#endif
...
#pragma omp master
...
#if DEBUG
                printf("Worker %d got A tile %d with value %.1f from rank %d using offset %d\n", rank*num_threads + tid, global_tile_a, local_ta[0], target_rank_a, target_offset_a);
#endif
...
#if DEBUG
                printf("Worker %d got B tile %d with value %.1f from rank %d using offset %d\n", rank*num_threads + tid, global_tile_b, local_tb[0], target_rank_b, target_offset_b);
#endif
#if COMPUTE
            /* compute Cij += Aik * Bkj */
            dgemm(local_ta, local_tb, local_tc, tile_dim);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
#if DEBUG
    printf("Rank %d done with the benchmark\n", rank);
#endif
    ...
    if (rank == 0) {
        ...
        if (!t_get_workers || !t_accum_workers || !t_get_flush_workers || !t_accum_flush_workers) {
            fprintf(stderr, "Unable to allocate memory for t_get_workers, t_accum_workers, t_get_flush_workers, or t_accum_flush_workers\n");
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
    } else
        t_get_workers = t_accum_workers = t_get_flush_workers = t_accum_flush_workers = NULL;
    ...
    if (rank == 0) {
        ...
        /* Assuming no operation would take more than 9999 seconds */
        ...
        mean_t_get_flush = sum_t_get_flush / nworkers_who_accumed;
    ...
    MPI_Win_sync(win_a);    /* MEM_MODE: synchronize private and public window copies */
    MPI_Win_sync(win_b);    /* MEM_MODE: synchronize private and public window copies */
...
#else
        /* This is the time for the whole kernel i.e. the observed time by the end user */
    ...
    if (rank == 0) {
        ...
        printf("Initializing C matrices\n");
        for (i = 0; i < mat_dim; i++) {
            for (j = 0; j < mat_dim; j++) {
               mat_c[i*mat_dim + j] = 0; 
        ...
        printf("Computing correct matrix\n");
#pragma omp parallel for
        ...
        printf("Done computing correct matrix\n");
